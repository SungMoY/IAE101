...
def construct_waltz():
    ...
    waltz = []
    for x in minuets:
        print("Playing minuet measure " + x)
        waltz.append(simpleaudio.WaveObject.from_wave_file(minuet_filename(x)))
    for x in trios:
        print("Playing trio measure " + x)
        waltz.append(simpleaudio.WaveObject.from_wave_file(trio_filename(x)))
    for x in waltz:
        x.play().wait_done()
    return -1
